...
public:
  ...
  DCO_ALWAYS_INLINE ~logging() {
    ...
    if constexpr (DCO_LOG_MAX_LEVEL >= 0) {
      if (_level <= DCO_LOG_MAX_LEVEL && _level <= level() && stream())
        output(os.str());
    }
  }
  ...
  DCO_ALWAYS_INLINE std::ostringstream& get_stream() {
    ...
    os << std::string(_level > logINFO ? static_cast<std::size_t>(_level - logINFO) : 0, '\t');
    return os;
  }
...
template class logging<dco_global_logging>;

#define DCO_LOGL(level)                                                                                                \
  if constexpr (DCO_LOG_MAX_LEVEL >= 0 && level <= DCO_LOG_MAX_LEVEL)                                                  \
...
inline std::string get_current_time() {
  ...
  std::snprintf(result, buf_size, "%s.%03ld", buffer, static_cast<long>(tv.tv_usec) / 1000);
  return result;
}
#endif
